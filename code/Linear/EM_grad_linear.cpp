...
// [[Rcpp::export]]
double ab(List mi, List zi, arma::mat ui, arma::mat sgammat, arma::mat seti, double d){
  int n = mi.size();
  NumericVector z(n);
  for(int i = 0; i<n;i++){
...
// [[Rcpp::export]]
arma::cube sampledi (List zi, List xi, int ti, double M, arma::mat Ut, arma::mat lambda, arma::mat Vt, List b, arma::mat sgammat, arma::mat seti, double d, double bo){
  // double count = 0;
  // double n= 30;
  ...
  for(int i=0; i< M-1;i++){
    ...
    /*check*/
    double a = ab(meandi, zi, u.slice(i), sgammat, seti, d);
    List meandi1 = Meani(meani,u.slice(i+1));
    double b = ab(meandi1, zi, u.slice(i+1),sgammat, seti , d);
    double R = exp(b-a);
...
// [[Rcpp::export]]
List sampleall( List A, List Xt, arma::vec t, double M, arma::mat Ut, arma::mat lambda, arma::mat Vt, List b, arma::mat sgammat, List set, double N, double d, double bo) {
...
// [[Rcpp::export]]
double qq(List Ai, List xi, int ti,double M, arma::mat Ut, arma::mat lambda, arma::mat Vt, List b, arma::mat sgammat, arma::mat seti, arma::cube gammai, double gam, double d, double bo) {
  int T = xi.size();
  List meani(T);
  ...
  for (int i = 0; i < gslice; i++){
    for (int j = 0; j < ti; j++){
      store(ti*i+j) =  -sum(mydnorm(matrix2vector(Ai[j]), matrix2vector(listM.slice(ti*i+j)), matrix2vector(seti)));
    }
  }
...
// [[Rcpp::export]]
double Q(List A, List Xt,arma::vec t, arma::mat Ut, arma::mat lambda, arma::mat Vt, List b, arma::mat sgammat, List set,List samples, double M, double gam, double d,double bo) {
  int N = A.length();
...
//  List xi(ti);changed 0617/24
    ...
    Q.slice(i) = qq(Ai, xi, ti, M, Ut,lambda, Vt, b, sgammat, seti, gammai, gam, d, bo);
  }
...
List mrglmm_id(List Y, List X, List A, List Xt, arma::vec t, double M, List bt, arma::mat sgamma0, arma::mat sgammatrue, List se0,  double tol, double maxit, double lhs, arma::mat Abar, arma::mat lambda, List b0, double tol1, double maxit1, double maxit2, double s, arma::mat utrue, arma::mat vtrue, List btrue, double gam, int r, double N, double p, double d,  double step1, double step2){
  ...
  while (eps > tol && iter < maxit) {
    ...
    double qfunction = Q(A, Xt, t, Ut, lambda, Vt, bt, sgammat,  set, samples,M, gam, d, 0);
...
//    Rcout << std::setprecision(10) <<"The eps at beginning of each EM is: " << eps << std::endl;
...
//    Rcout << std::setprecision(10) <<"The abs(lhs - approxq)/abs(approxq) is: " << label << std::endl;
 ...
 //     Rcout  << "The approxq at beginning of inner 1 is: " << approxq << std::endl;
      ...
      for (int q = 0; q < bn; ++q) {
        ...
        for (int i = 0; i < N; ++i) {
          ...
          for (int T = 0; T < t(i); ++T) {
            ...
            hu -= (((Yt.slice(i)-fm) / (seti % seti)) * Vt) / (denom*bn) - (4 * gam * Ut * (trans(Ut) * Ut - trans(Vt) * Vt)) / (denom *bn);
            hv -= (trans((Yt.slice(i)-fm) / (seti % seti)) * Ut) / (denom*bn) - (4 * gam * Vt * (trans(Vt) * Vt - trans(Ut) * Ut)) / (denom *bn);
      ...
      while (Q(A, Xt, t, U0 - a * hu, lambda, V0 - a * hv, bt, sgammat,set,  samples, M, gam, d, 0) - approxq - (1/(2.0*a)*Res2) > 0) {
        arma::vec new_rowt(1);
        a *= 0.5;
        new_rowt(0) = a;  
...
//      Rcout << "The updated V of inner 1 is: " << Vu << std::endl;
      ...
      if (s >0){
      while (Q(A, Xt, t, Ut,lambda, Vt, trunb, sgammat, set, samples, M, gam, d, 0) - approxq - (1/(2.0*c)*Res4)  > 0){
        arma::vec new_rowb(1);
        c *= 0.8;
        new_rowb(0) = c;  
...
//  Rcout << std::fixed<< std::setprecision(10) <<"The U0k is: " << U0 << std::endl;
  ...
  while (Eps > tol && iter < maxit2) {
    // double qfunction0 = lhs;
    List samples = sampleall(A, Xt, t, M, Uk, lambda, Vt, bk, sgammat,set,  N, d, 1);
    double qfunction = Q(A, Xt,t, Uk,lambda, Vt, bk, sgammat, set, samples, M, gam, d, 1);
//    Rcout <<"The result of Q is: " << qfunction << std::endl;
...
//      Rcout << std::fixed << std::setprecision(5) << "The eps" << (i + 2) << " is: " << Epsa(i) << std::endl;
 ...
 //   Rcout  <<"The iter1 is: " << iter1 << std::endl;
    ...
    while (std::abs(lhs - approxq)/std::abs(approxq) > tol1 && iter1 < maxit1) {
      approxq = Q(A, Xt, t, Uk, lambda, Vt, bk, sgammat, set,  samples, M, gam, d,1);
//      Rcout  << "The approxq at beginning of inner 2 is: " << approxq << std::endl;
      ...
      for (int q = 0; q < bkn; ++q) {
        ...
        for (int i = 0; i < N; ++i) {
          ...
          for (int T = 0; T < t(i); ++T) {
            ...
            arma::vec outer_xqi = xq.subvec(i*t(i),i*t(i)+t(i)-1);
            double outer_xqit = outer_xqi(T);
            hu -= (((Yt.slice(i)-fm) / (seti % seti)) * Vt) / (denom*bkn) ;
            hv -= (trans((Yt.slice(i)-fm) / (seti % seti)) * Ut) / (denom*bkn);
            // Calculate the adjustment for hb[q] for this specific index i
      ...
      while (Q(A, Xt, t, U0 - a * hu, lambda, V0 - a * hv, bk, sgammat, set, samples, M, gam, d, 0) - approxq -  (1/(2.0*a)*Res2) > 0) {
...
//        Rcout << "The Record_step_theta of inner 2 is: " << Record_step_theta << std::endl;
      ...
      // Rcout << "The updated U of inner 1 is: " << Uu << std::endl;
      if (s>0){
      while (Q(A, Xt, t, Uk,lambda, Vt, trunb, sgammat, set,samples, M, gam, d, 0) - approxq - (1/(2.0*c)*Res4)  > 0){
        arma::vec new_rowb(1);
        c *= 0.8;
        new_rowb(0) = c;  
        Record_step_b = join_cols(Record_step_b, new_rowb);
//        Rcout << "The Record_step_b of inner 2 is: " << Record_step_b << std::endl;
      ...
      lhs = Q(A, Xt,t, Uk, lambda, Vt, bk, sgammat, set, samples, M, gam, d, 1);
