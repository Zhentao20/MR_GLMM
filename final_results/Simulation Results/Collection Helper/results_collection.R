main_folder <- '~/Desktop/final_results/linear'
library(dplyr)
library(xtable)
...
for (i in 1:length(naive_scad_files)) {
  identifier <- sub(".*/(linear_[^/]+)/naive/.*", "\\1", naive_scad_files[i])
...
latex_table <- xtable(result_naive_scad)

# Print the LaTeX code for the table
print(latex_table, type = "latex")
write.csv(result_naive_scad, file = "~/Desktop/final_results/linear_result_naive_scad.csv", row.names = FALSE)
output_file <- read.csv("~/Desktop/final_results/linear_result_naive_scad.csv")
print(output_file)
...
for (i in 1:length(naive_lasso_files)) {
  identifier <- sub(".*/(linear_[^/]+)/naive/.*", "\\1", naive_lasso_files[i])
...
write.csv(result_naive_lasso, file = "~/Desktop/final_results/linear_result_naive_lasso.csv", row.names = FALSE)
output_file1 <- read.csv("~/Desktop/final_results/linear_result_naive_lasso.csv")
print(output_file1)
...
for (net_file in net_files) {
  ...
  identifier <- sub(".*/(linear_[^/]+)/net/.*", "\\1", net_file)
...
write.csv(result_net, file = "~/Desktop/final_results/linear_result_net.csv", row.names = FALSE)
output_file2 <- read.csv("~/Desktop/final_results/linear_result_net.csv")
print(output_file2)

# Assuming the three data frames are: result_naive_scad, result_naive_lasso, result_net

# Add method column (if not already done)
